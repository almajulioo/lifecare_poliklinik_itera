...
@section('page_title', 'Tambah Obat Baru')
...
<div class="space-y-8">
    ...
    <!-- Form Section -->
    <x-admin.card>
        <h2 class="text-xl font-bold text-gray-900 mb-6">Formulir Tambah Obat</h2>
        ...
        <form action="{{ route('admin.obat.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="unit" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        ...
                        <option value="kapsula" {{ old('unit') === 'kapsula' ? 'selected' : '' }}>Kapsul</option>
            ...
            <div>
                ...
                <textarea name="notes" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Contoh: Gunakan setelah makan, hindari pada ibu hamil...">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
